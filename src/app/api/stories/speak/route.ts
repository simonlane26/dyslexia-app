...
// Max ~10,000 chars — enough for any story length at level 5
...
function extractWordTimings(text: string, alignment: ElevenLabsAlignment): WordTiming[] {
  ...
  while (i < text.length) {
    // Skip whitespace / section breaks
    ...
    // Guard against alignment array being shorter than text
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const wordTimings = extractWordTimings(text, alignment);

    return NextResponse.json({ audioBase64, wordTimings });
  } catch (e) {
